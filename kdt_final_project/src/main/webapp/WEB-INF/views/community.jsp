					...
					<c:if test="${board.board_title == '여행기록' }">
						<li id="top${board.id}">
							...
							<div class="writerinfo">
								<span class="profile"></span> <span class="writer">${board.writer}</span>
							</div>
							<div class="info">
								<span class="views">views ${board.views}</span> <span
									class="likeCount">like ${board.likecount}</span> <span
									class="wirteTime">${board.writingtime}</span>
							</div>
						</li>
					</c:if>
				</c:forEach>
			...
			<table>
				...
				<tbody>
					<c:forEach items="${boardList}" var="board" varStatus="loop">
						<c:if
							test="${board.board_title == '여행기록' && !top10Ids.contains(board.id) && loop.index >= 10}">
							<tr>
								<td>${board.id}</td>
								<td>${board.place}</td>
								<td><a
